...
<div class="p-10">
  <div class="dropdown inline-block relative">
    ...
    {#if dropDownOpen}
      <div
        class="dropdown-menu absolute hidden mt-1 w-full rounded-md bg-white shadow-lg z-10"
      >
        <ul
          tabindex="-1"
          role="listbox"
        ...
        >
          {#each languages as language}
            <li
              id="listbox-item-0"
              role="option"
              class="text-gray-900 cursor-default select-none relative py-2 pl-3 pr-9 hover:bg-gray-400"
              on:click="{() => changeLanguage(language.language)}"
            >
              <div class="flex items-center">
                <img
                  src="{liFlag(language.language)}"
                  alt=""
                  class="flex-shrink-0 h-6 w-6 rounded-full"
                />
